...
// }
if (!($mysql_host)) {
    die(install_json_msg('mysqlHost'));
}

if (!($mysql_user)) {
    die(install_json_msg('mysqlUser'));
}

if (!($mysql_pass)) {
    die(install_json_msg('mysqlPass'));
}
if (!($mysql_db)) {
    die(install_json_msg('mysqlDB'));
}
if (!($table_prefix)) {
    die(install_json_msg('table_prefix'));
}
// if(!$bool_ajax_token){
